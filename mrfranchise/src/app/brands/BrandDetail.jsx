...
const ExpansionLocationTags = lazy(
  () => import("@/Components/BrandViewPageHandling/ExpansionLocationTags.jsx"),
);
const ImageDialog = lazy(
  () => import("@/Components/BrandViewPageHandling/ImageDialogBoxViewPage.jsx"),
);
const ApplyDrawer = lazy(
  () =>
    import("@/Components/BrandViewPageHandling/ApplyDrawerFormViewPage.jsx"),
);
const BackToTopButton = lazy(
  () =>
    import("@/Components/BrandViewPageHandling/BackToTopButtonViewPage.jsx"),
);
const FloatingApplyButton = lazy(
  () =>
    import("@/Components/BrandViewPageHandling/FloatingApplyButtonViewPage.jsx"),
);
const SimilarBrands = lazy(
  () => import("@/Components/HomePage_VideoSection/SimilarBrands.jsx"),
);
const LikedBrands = lazy(
  () => import("@/Components/HomePage_VideoSection/LikedBrands.jsx"),
);
const ViewBrands = lazy(
  () => import("@/Components/HomePage_VideoSection/ViewBrands.jsx"),
);
const ShortListedBrands = lazy(
...
const BrandDetails = ({ brandData }) => {
  ...
  const investmentRanges = useMemo(
    () => [
      ...new Set(
        selectedBrand[0]?.brandfranchisedetails?.franchiseDetails?.fico?.map(
          (m) => m.investmentRange,
        ) || [],
      ),
    ],
  ...
  const BrandName = useMemo(() => {
    return selectedBrand[0]?.brandDetails?.brandName || "Franchise";
  }, [selectedBrand]);

  const BrandIndustry = useMemo(() => {
    return (
      selectedBrand[0]?.brandfranchisedetails?.franchiseDetails?.brandCategories
        ?.main || "Industry"
    );
  }, [selectedBrand]);
  ...
  const BrandFranchiseModel = useMemo(() => {
    return (
      selectedBrand[0]?.brandfranchisedetails?.franchiseDetails?.fico?.[0]?.franchiseModel
    );
  }, [selectedBrand]);

  const brandOriginState = useMemo(() => {
    return selectedBrand[0]?.brandDetails?.state
  }, [selectedBrand]);
  const brandOriginDistrict = useMemo(() => {
    return selectedBrand[0]?.brandDetails?.district
     
  }, [selectedBrand]);

  const brandOriginCity = useMemo(() => {
    return selectedBrand[0]?.brandDetails?.city
    ...
    async (e) => {
      ...
      try {
        ...
        if (response.data && response.data.success) {
          ...
          const brandName =
            selectedBrand[0]?.brandDetails?.brandName || "Franchise";
...
https://mrfranchise.in`,
  ...
  useEffect(() => {
    const locations =
      selectedBrand[0]?.brandexpansionlocationdatas?.expansionLocations
        ?.domestic?.locations || [];
    ...
    <>
      ...
      >
        ...
        <MediaSection
          ...
          isTablet={isTablet}
          brandName={BrandName}
            brandOriginState={brandOriginState}
          brandOriginDistrict={brandOriginDistrict}
          brandOriginCity={brandOriginCity}
          brandFranchiseModel={BrandFranchiseModel}
