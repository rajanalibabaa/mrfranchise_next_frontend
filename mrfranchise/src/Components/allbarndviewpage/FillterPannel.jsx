    ...
    <>
      {parts.map((part, i) =>
        part.toLowerCase() === highlight.toLowerCase() ? (
          <span key={i} style={{ color: "#ff9800", fontWeight: 600, fontSize: "inherit", lineHeight: "inherit" }}>
            {part}
          </span>
        ) : (
          part
...
const DropdownSearchBox = ({ value, onChange, onClear, placeholder, inputRef }) => (
  ...
  >
    <Box sx={{
      ...
      border: "1.5px solid #ff9800", borderRadius: "8px",
      px: 1.2, py: 0.6, gap: 1, backgroundColor: "#fff",
    }}>
      ...
      {value && (
        <Close
          ...
          sx={{ color: "#aaa", fontSize: 18, cursor: "pointer", flexShrink: 0, "&:hover": { color: "#ff9800" } }}
        />
      )}
    </Box>
  </Box>
...
const HeadingLabel = ({ children, color = "text.secondary", bg = "#f5f5f5" }) => (
  <Box sx={{
    px: 1, py: 0.5,
    fontSize: "0.65rem", fontWeight: 700,
    textTransform: "uppercase", letterSpacing: "0.08em",
    color, backgroundColor: bg,
    borderRadius: "6px", mt: 1.5, mb: 0.5,
    pointerEvents: "none",
    width: "95%",
  }}>
    {children}
...
}, [expandedSections.modelType, filters.modelType, activeFranchiseModel, franchiseModels, dispatch]);
...
useEffect(() => {
  ...
  const toggleSection = (section) => {
    setExpandedSections((prev) => {
      const next = { ...prev };
      if (!prev[section]) {
        Object.keys(next).forEach((k) => { if (k !== section) next[k] = false; });
      }
  ...
  const filteredMainCategories = useMemo(() => {
    ...
    for (const group of mainCategories) {
      ...
      if (matched.length > 0) result.push({ heading: group.heading, industries: matched });
    }
    return result;
  ...
  const filteredFranchiseHeading = useMemo(() => {
    ...
    for (const [heading, types] of Object.entries(franchiseHeading || {})) {
      ...
      const matchedTypes = types.filter((t) => t.toLowerCase().includes(term));
  ...
  return (
    <Box sx={{ pr: 2, height: "calc(100vh - 120px)", width: "100%", overflowY: "auto" }}>
      ...
      <Typography variant="body2" sx={{
        color: "#000", background: "#7cd13b", display: "block",
        textAlign: "center", padding: "10px", borderRadius: "5px", mb: 1, mt: 4,
      }}>
      ...
      >
        <AccordionSummary expandIcon={<ExpandMoreIcon sx={{ color: "#4caf50" }} />}
          ...
          <Typography sx={{ color: "#4caf50", fontWeight: "bold", fontSize: "0.875rem" }}>
            Industries
            {filters.maincat && (
              <Box component="span" sx={{ ml: 1, fontSize: "0.7rem", color: "#ff9800", fontWeight: 400 }}>
                ({filters.maincat})
              </Box>
            )}
          </Typography>
                ...
                <React.Fragment key={`heading-${group.heading}`}>
                  ...
                  {group.industries.map((industry) => (
                    <Box key={`cat-container-${industry}`}>
                      <FormControlLabel
                        value={industry}
                        control={<Radio size="small" sx={{ color: "#ff9800", "&.Mui-checked": { color: "#4caf50" }, padding: "6px" }} />}
                        label={
                          <Typography fontSize="0.8125rem">
                            <HighlightedText text={`${industry} Franchise`} highlight={searchTerms.mainCategory} />
                          </Typography>
                        }
                        sx={{ mb: 0, mr: 0 }}
                      />
                            ...
                            {filteredSubCategories.map((subCategory) => (
                              <Box key={`subcat-container-${subCategory}`} sx={{ mt: 1 }}>
                                <FormControlLabel
                                  ...
                                  label={
                                    <Typography fontSize="0.8125rem">
                                      <HighlightedText text={`${subCategory} Franchise`} highlight={searchTerms.subCategory} />
                                    </Typography>
                                  }
                                  sx={{ mb: 0, mr: 0 }}
                                />
                                      ...
                                      >
                                        {childCategories.map((child) => (
                                          <FormControlLabel
                                            ...
                                            value={child}
                                            control={<Radio size="small" sx={{ color: "#ff9800", "&.Mui-checked": { color: "#4caf50" }, padding: "6px" }} />}
                                            label={<Typography fontSize="0.8125rem">{child}</Typography>}
                                            sx={{ mb: 0.5, mr: 0 }}
                                          />
                                        ))}
                                      </RadioGroup>
...
>
  ...
  <AccordionDetails sx={{ p: 0 }}>
    ...
    {/* Top Level */}
    <Box sx={{ px: 1.5, pt: 1, pb: 0.5, borderBottom: "1px solid #f5f5f5" }}>
      <Typography sx={{ fontSize: "0.7rem", color: "text.secondary", mb: 0.5, textTransform: "uppercase" }}>
        Select Model
      </Typography>
      <RadioGroup
          ...
          <>
            ...
            <Box sx={{ px: 1, maxHeight: 340, overflowY: "auto" }}>
              <RadioGroup
                ...
                onChange={(e) => {
                  console.log("FranchiseType selected:", e.target.value);
                  onFilterChange("franchiseType", e.target.value);
                }}
              >
                {Object.entries(filteredFranchiseHeading).map(([heading, types]) => (
                  <React.Fragment key={heading}>
                    <Box sx={{ px: 1, py: 0.5, fontSize: "0.65rem", fontWeight: 700, textTransform: "uppercase", color: "#ff9800", backgroundColor: "#fff8f0", borderRadius: "4px", mt: 1.5, mb: 0.5 }}>
                      {heading}
                    </Box>
                    {types.map((type) => (
                      <FormControlLabel
            ...
            >
              {filteredInvestmentRanges.map((range) => (
                <FormControlLabel
                  ...
                  value={range}
                  control={<Radio size="small" sx={{ color: "#ff9800", "&.Mui-checked": { color: "#4caf50" }, padding: "6px" }} />}
                  label={<Typography fontSize="0.8125rem"><HighlightedText text={range} highlight={searchTerms.investmentRange} /></Typography>}
                  sx={{ mb: 0, mr: 0 }}
                />
              ))}
            </RadioGroup>
            ...
            >
              ...
              {filteredAreaRequired.map((area) => (
                <FormControlLabel
                  ...
                  value={area}
                  control={<Radio size="small" sx={{ color: "#ff9800", "&.Mui-checked": { color: "#4caf50" }, padding: "6px" }} />}
                  label={<Typography fontSize="0.8125rem"><HighlightedText text={area} highlight={searchTerms.areaRequired} /></Typography>}
                  sx={{ mb: 0, mr: 0 }}
                />
              ))}
            </RadioGroup>
      ...
      >
        <AccordionSummary expandIcon={<ExpandMoreIcon sx={{ color: "#4caf50" }} />}
          ...
          <Typography sx={{ color: "#4caf50", fontWeight: "bold", fontSize: "0.875rem" }}>
            ...
            {filters.state && (
              <Box component="span" sx={{ ml: 1, fontSize: "0.7rem", color: "#ff9800", fontWeight: 400 }}>
                ({filters.state}{filters.district ? ` › ${filters.district}` : ""}{filters.city ? ` › ${filters.city}` : ""})
              </Box>
            )}
          </Typography>
        </AccordionSummary>
        ...
        <AccordionDetails sx={{ p: 0 }}>
          ...
          <Box sx={{ px: 1, maxHeight: 360, overflowY: "auto" }}>
            {loading ? (
              <Box sx={{ p: 2 }}><CircularProgress size={20} sx={{ color: "#ff9800" }} /></Box>
            ) : (
              <RadioGroup
                value={filters.state || ""}
                onChange={(e) => {
                ...
                {filteredStates.map((state) => (
                  <Box key={`state-box-${state}`} sx={{ mb: 0.5 }}>
                    <FormControlLabel
                      value={state}
                      control={<Radio size="small" sx={{ color: "#ff9800", "&.Mui-checked": { color: "#4caf50" }, padding: "6px" }} />}
                      label={<Typography fontSize="0.8125rem"><HighlightedText text={state} highlight={searchTerms.state} /></Typography>}
                      sx={{ mb: 0, mr: 0 }}
                    />
                            ...
                            >
                              ...
                              {filteredDistricts.map((district) => (
                                <FormControlLabel
                                  ...
                                  value={district}
                                  control={<Radio size="small" sx={{ color: "#ff9800", "&.Mui-checked": { color: "#4caf50" }, padding: "6px" }} />}
                                  label={<Typography fontSize="0.8125rem"><HighlightedText text={district} highlight={searchTerms.district} /></Typography>}
                                  sx={{ mb: 0.5, mr: 0 }}
                                />
                              ))}
                            </RadioGroup>
                                  ...
                                  >
                                    {filteredCities.map((city) => (
                                      <FormControlLabel
                                        ...
                                        value={city}
                                        control={<Radio size="small" sx={{ color: "#ff9800", "&.Mui-checked": { color: "#4caf50" }, padding: "6px" }} />}
                                        label={<Typography fontSize="0.8125rem"><HighlightedText text={city} highlight={searchTerms.city} /></Typography>}
                                        sx={{ mb: 0.5, mr: 0 }}
                                      />
                                    ))}
                                  </RadioGroup>
