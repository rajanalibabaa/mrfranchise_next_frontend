...
const usePackageActions = (dataHook, onAddInvestmentRange) => {
      ...
      data.packages.forEach((packageItem) => {
        ...
        investPackages.forEach((investPackage) => {
          (investPackage.investmentranges || []).forEach((range) => {
            ...
            (investPackage.selectedPlanStateAndDistrict || investPackage.SelectedPlanStateAndDistrict || [])
              .forEach((entry) => { if (entry.state) selectedInOtherRanges.add(entry.state); });
  ...
  const handleCloseStateModal = useCallback(() => { setOpenStateModal(false); }, [setOpenStateModal]);
  ...
  const handleOpenStateModal = useCallback(
    (investmentRangeLabel, range, planId = null) => {
          ...
          investPackages.forEach((investPackage) => {
            (investPackage.investmentranges || []).forEach((r) => {
              ...
              if (existingRange === editingRangeValue) {
                (investPackage.selectedPlanStateAndDistrict || investPackage.SelectedPlanStateAndDistrict || [])
                  .forEach((entry) => { if (entry.state) purchasedStatesForThisRange.push(entry.state); });
                (r.selectedPlanStateAndDistrict || [])
                  .forEach((entry) => { if (entry.state) purchasedStatesForThisRange.push(entry.state); });
              } else {
                (investPackage.selectedPlanStateAndDistrict || investPackage.SelectedPlanStateAndDistrict || [])
                  .forEach((entry) => { if (entry.state) otherRangeStates.add(entry.state); });
                (r.selectedPlanStateAndDistrict || [])
                  .forEach((entry) => { if (entry.state) otherRangeStates.add(entry.state); });
              }
            });
      ...
      const committedItem = paymentSummary.flatMap((g) => g.items).find((item) =>
      ...
      let statesToPreselect = finalToken
        ...
        : (committedItem?.states || savedStates || (detectedState ? [detectedState] : []));

      if (!statesToPreselect?.length) {
        statesToPreselect = allStates.length > 0 ? allStates.filter((s) => !otherRangeStates.has(s)) : [];
      }

      setSelectedStates(statesToPreselect?.length > 0 ? new Set(statesToPreselect) : new Set());
      setOpenStateModal(true);
    },
    [getRangeKey, paymentSummary, statesByInvestmentRange, finalToken, detectedState, allStates, data,
     setCurrentEditingRange, setSelectedStates, setOpenStateModal],
  );
  ...
  const handleSaveStates = useCallback(() => {
    ...
    const updated = isAllStatesSelected
      ? (() => { const u = { ...statesByInvestmentRange }; delete u[currentEditingRange]; return u; })()
      ...
      prev.map((group) => {
        ...
        const updatedItems = group.items.map((item) => {
          ...
          return {
            ...
            totalAmount: (group.pricePerState / divisor) * statesToUse.length * (item.selectedLeads || 0),
          };
        });
        ...
        return {
          ...
          amount: (group.pricePerState / divisor) * totalUniqueStates * selectedLeads,
    ...
    selectedStates, statesByInvestmentRange, currentEditingRange, getRangeKey,
    leadsDropdownData, openSnack, handleCloseStateModal, getAlreadySelectedStatesInOtherRanges,
    allStates, finalToken, setStatesByInvestmentRange, statesByInvestmentRangeRef, setPaymentSummary,
  ]);
  ...
  }, [getStatesToDisplay, getAlreadySelectedStatesInOtherRanges, openSnack, setSelectedStates]);

  const handleClearAll = useCallback(() => {
    const blocked = getAlreadySelectedStatesInOtherRanges();
    setSelectedStates((prev) => {
      const next = new Set();
      prev.forEach((state) => { if (blocked.has(state)) next.add(state); });
      return next;
    });
  ...
  const handleAddSingleToPayment = useCallback(
    (item, selectedPlan, selectedPkg) => {
      ...
      let selectedLeads =
        item.selectedLeads ||
        selectedLeadsPerRange[`plan-${selectedPlan._id}-${investmentRangeLabel}`] || 0;
      ...
      const divisor = minLeads > 0 ? minLeads : 1;
      const newItem = {
        id, investmentRangeLabel, range,
        stateCount: states.length, states, selectedLeads,
        ...
        if (existingGroup) {
          ...
          const updatedItems = existingItemIndex !== -1
            ? existingGroup.items.map((it) => ({
                ...
                totalAmount: (pricePerState / divisor) * it.stateCount * selectedLeads,
                ...(it.id === newItem.id ? { states: newItem.states, stateCount: newItem.stateCount } : {}),
              }))
          ...
          newSummary = prev.map((g) =>
            g.groupKey === groupKey
              ? { ...g, items: updatedItems, uniqueStates, totalStates: totalUniqueStates, amount: (pricePerState / divisor) * totalUniqueStates * selectedLeads, totalLeads: totalUniqueStates * selectedLeads }
              : g
          );
        } else {
          ...
          newSummary = [
            ...prev,
            {
              ...
              planPackageId: selectedPkg._id, investmentRangeLabel,
              validityDays: selectedPkg?.validityDays, pricePerState,
              uniqueStates, totalStates: totalUniqueStates,
              amount: (pricePerState / divisor) * totalUniqueStates * selectedLeads,
              totalLeads: totalUniqueStates * selectedLeads,
              selectedLeads, items: [newItem],
            },
          ];
        }
    ...
    [
      getRangeKey, finalToken, detectedState, allStates, getUniqueStatesAcrossRanges,
      openSnack, leadsDropdownData, selectedLeadsPerRange, scrollToPaymentSummary,
      statesByInvestmentRangeRef, setPaymentSummary, setMovedGroupKeys, setSelected, setCheckedItems,
    ],
  );
  ...
  const handleRemoveSingleFromPayment = useCallback(
    (item) => {
      ...
      setPaymentSummary((prev) => {
        const updated = prev.map((g) => {
          ...
          const selectedLeads = updatedItems[0]?.selectedLeads || 0;
          return {
            ...g, items: updatedItems, uniqueStates: newUniqueStates,
            totalStates: newUniqueStates.length,
            amount: (g.pricePerState / divisor) * newUniqueStates.length * selectedLeads,
    ...
    [getUniqueStatesAcrossRanges, openSnack, leadsDropdownData, setPaymentSummary, setMovedGroupKeys],
  );
  ...
  const handleLeadsChange = useCallback(
    (planGroupKey, newLeadsValue) => {
      ...
      setPaymentSummary((prev) =>
        prev.map((group) => {
          if (group.isListingPlan || group.planId !== actualPlanId) return group;
          if (!group.items.some((item) => item.range === specificRange)) return group;
          ...
          const divisor = minLeads > 0 ? minLeads : 1;
          const updatedItems = group.items.map((item) =>
            item.range !== specificRange ? item : {
              ...item, selectedLeads: newLeadsValue,
          ...
          const totalUniqueStates = uniqueStates.length;
          return {
            ...group, items: updatedItems, uniqueStates, totalStates: totalUniqueStates,
            amount: (group.pricePerState / divisor) * totalUniqueStates * newLeadsValue,
    ...
    [leadsDropdownData, movedGroupKeys, openSnack, setSelectedLeadsPerRange, setPaymentSummary],
  );
    ...
    paymentGroups.forEach((group) => {
      if (!plansMap.has(group.planId)) {
        plansMap.set(group.planId, {
          packagesType: group.packagesType, packagesName: group.planName,
          planUniqueId: group.planId, planPackageId: group.planPackageId,
          InvestmetPackages: [],
        });
      }
      ...
      group.items.forEach((item) => {
        plan.InvestmetPackages.push(
          item.isListingPlan
            ? { PackageName: item.investmentRangeLabel, Amount: group.pricePerState, Validity: group.validityDays, TotalLeads: "-", States: item.states || ["ALL STATES"], InvestmentRange: item.range, InvestmentRangeLabel: item.investmentRangeLabel, LeadsPerState: "-" }
            : { PackageName: item.investmentRangeLabel, Amount: group.pricePerState, Validity: group.validityDays, TotalLeads: item.selectedLeads * (item.stateCount || 0), States: item.states || [], InvestmentRange: item.range, InvestmentRangeLabel: item.investmentRangeLabel, LeadsPerState: item.selectedLeads }
        );
      });
    });
  ...
  const handleProceedToPayment = useCallback(() => {
    ...
    router.push("/payment");
  }, [finalToken, openSnack, paymentSummary, movedGroupKeys, router, transformPaymentToAPIFormat, setShowLogin]);
