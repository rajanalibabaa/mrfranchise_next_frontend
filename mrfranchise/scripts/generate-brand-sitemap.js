...
async function fetchAllBrands() {
  ...
  while (true) {
    const result = await fetchBrandPage(page, 100);

    if (result.brands.length === 0) break;

    all.push(...result.brands);

    if (result.brands.length < 100) break;
...
async function generate() {
  ...
  for (const brand of brands) {
    ...
    const slug = brand.slug ? slugify(brand.slug) : slugify(name);
...
`);
  ...
  fs.writeFileSync("./public/sitemap-brands.xml", xml);

  console.log(`DONE: ${urls.length} brand URLs created`);
}
